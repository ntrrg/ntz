...
pub const Level = enum {
    ...
    pub fn asLog(lvl: Level, writer: anytype, enc: ctxlog.Encoder) !void {
        if (enc.options.prefix.len > 0) {
            _ = try writer.write(enc.options.prefix);
            _ = try writer.write("=");
        }

        _ = try writer.write("\"");
        _ = try writer.write(lvl.asKey());
        _ = try writer.write("\"");
    }
