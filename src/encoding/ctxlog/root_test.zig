...
test "ntz.encoding.ctxlog.encode" {
    ...
    try testing.expectErr(e.encode(w, mp), ctxlog.Encoder.EncodeError.InvalidPointer);
    buf.clear();

    const cp: [*c]const u8 = "hello, world!";
    try testing.expectErr(e.encode(w, cp), ctxlog.Encoder.EncodeError.InvalidPointer);
    buf.clear();
    ...
    try e.encode(w, person);
    try testing.expectEqlStrs(buf.bytes(), "name=\"Miguel Angel\" height=187 skills=[name=\"Zig\" prof=\"beginner\", name=\"Go\" prof=\"average\", name=\"Breathing\" prof=\"pro\", name=\"Eating\" prof=\"god\"] pet=null");
    buf.clear();
