...
test "ntz.types.structs.setField" {
    const Point = struct {
        name: [:0]const u8,
        x: usize,
        y: usize,
    ...
    var orig = structs.init(Triangle);
    try testing.expectEql(orig.h.a.x, 0);
    structs.setField(&orig, "h.a.x", 1);
    try testing.expectEql(orig.h.a.x, 1);
    try testing.expectEqlStrs(orig.h.a.name, "");
    structs.setField(&orig, "h.a.name", "HypLinePointA");
    try testing.expectEqlStrs(orig.h.a.name, "HypLinePointA");
    structs.setField(&orig, "h.a", .{ .name = "", .x = 12, .y = 3 });
    try testing.expectEqlStrs(orig.h.a.name, "");
    try testing.expectEql(orig.h.a.x, 12);
    try testing.expectEql(orig.h.a.y, 3);
}
