...
pub fn Field(comptime T: type, comptime fp: []const u8) type {
    ...
    loop: while (true) {
        for (fields(field_type)) |f| {
            ...
            @compileError("no field '" ++ field_name ++ "' on type " ++ @typeName(field_type));
        }
    }
...
/// path.
///
/// This is equivalent to `x.a` or `x.a.b`, but it cannot be done
/// programmatically.
///
/// A field path is a list of fields separated by periods (`.`). All elements
/// in the field path must be structs or unions, except for the last one.
...
/// field path.
///
/// This is equivalent to `x.a = val` or `x.a.b = val`, but it cannot be done
/// programmatically.
///
/// A field path is a list of fields separated by periods (`.`). All elements
/// in the field path must be structs or unions, except for the last one.
