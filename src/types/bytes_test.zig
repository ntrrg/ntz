...
test "ntz.types.slices.isString" {
    // Slices.
    ...
    const vec: @Vector(arr.len, u8) = arr;
    try testing.expect(bytes.isString(vec));

    const vec32: @Vector(arr32.len, u32) = arr32;
    try testing.expect(!bytes.isString(vec32));
    ...
    try testing.expect(!bytes.isString(true));
    try testing.expect(!bytes.isString(123));
    try testing.expect(!bytes.isString(1.23));
    try testing.expect(!bytes.isString(.some_enum));
}
