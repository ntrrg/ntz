...
pub fn Iterator(comptime T: type) type {
    return struct {
        ...
        pub fn next(it: *Self) ?T {
            if (it.index + 1 >= it.data.len) return null;
            it.index += 1;
            return it.data[it.index];
        }

        pub fn peek(it: Self, n: usize) ?T {
            const i = it.index + n;
        ...
        pub fn skip(it: *Self) bool {
            if (it.index + 1 >= it.data.len) return false;
            it.index += 1;
            return true;
        }
    };
...
pub fn iterator(s: anytype) Iterator(Child(@TypeOf(s))) {
    return .{ .data = s };
}
