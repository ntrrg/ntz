...
pub fn Buffer(comptime Allocator: type) type {
    return struct {
        ...
        pub const Error = WriteError || AllocatorError;
        pub const AllocatorError = errors.From(Allocator);

        ally: Allocator,
        data: slices.Slice(u8, void),

        pub fn deinit(buf: Self) void {
            buf.data.deinitAlloc(buf.ally);
        }
        ...
        pub fn write(buf: *Self, data: []const u8) WriteError!usize {
            try buf.data.appendManyAlloc(buf.ally, data);
            return data.len;
        }

        /// Creates a simplified writer using the buffer as output.
        pub fn writer(buf: *Self) io.Writer(*Self, WriteError, write) {
            return .{ .writer = buf };
        }
    };
}
