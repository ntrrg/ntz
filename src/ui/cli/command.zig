...
pub fn Command(
    ...
    return struct {
        ...
        pub fn load(
            ...
            for (cmd.opts.items()) |opt| {
                opt.load(arena, cmd, ctx, entries) catch |err| {
                    ...
                    cmd.log.with("error", err).errf(arena, msg, .{opt.id});
                    return err;
                };
            }
            ...
            for (entries) |entry| {
                ...
                for (cmd.cmds.items()) |*sub_cmd| {
                    ...
                    return sub_cmd.load(arena_allocator, ctx, entries) catch |err| {
                        const msg = "cannot load option entries for subcommand '{s}'";
                        cmd.log.with("error", err).errf(arena, msg, .{sub_cmd.name});
                        return err;
                    };
                }
            }
        ...
        pub const configStructOption: cli.Option(Self, Context) = .{
            .id = "config_struct",
            .flags = &.{"--gen-config-struct"},
            .help = "Generates a struct for parsing configuration files",
            .placeholder = "file",
            .has_optional_value = true,
            .readAction = Self.configStructAction,
            .action = Self.emptyAction,
        };
        ...
        pub fn configStructAction(
            ...
            try cli.writeConfigStruct(io.stdout(), cmd, 0);
            std.process.exit(0);
        }
        ...
        pub fn envFileReadAction(
            ...
            cmd.fromEnvFile(arena, entries, name) catch |err| {
                ...
                cmd.log.with("error", err).errf(arena, msg, .{name});
                return err;
            };
        }
        ...
        pub fn fromOS(cmd: Self, ctx: *Context) !u8 {
            ...
            defer for (entries.items()) |entry| {
                if (entry.key.len > 0) arena.free(entry.key);
                arena.free(entry.value);
            };
            ...
            return cmd.load(&arena_ally, ctx, entries.items());
        }
        ...
        pub fn fromStruct(
            ...
            inline for (ti.fields) |field| {
                ...
                cmd.fromStructField(arena, entries, encoder, field.name, field_value) catch |err| {
                    const msg = "cannot read configuration entry '" ++ field.name ++ "'";
                    cmd.log.with("error", err).err(msg);
                    return err;
                };
            }
        ...
        pub fn fromStructField(
            ...
            switch (ti) {
                .@"struct" => {
                    for (cmd.cmds.items()) |sub_cmd| {
                        ...
                        sub_cmd.fromStruct(arena, entries, encoder, value) catch |err| {
                            const msg = "cannot read configuration for subcommand '{s}'";
                            cmd.log.with("error", err).errf(arena, msg, .{sub_cmd.name});
                            return err;
                        };
                    ...
                    try encoder.encode(buf.writer(), value);
                },
                ...
                else => try encoder.encode(buf.writer(), value),
            }

            for (cmd.opts.items()) |opt| {
                if (bytes.equal(name, opt.config)) break;
            } else {
                const msg = "unknown configuration entry '" ++ name ++ "'";
                cmd.log.err(msg);
                return error.UnknowConfig;
            }
            ...
            try entries.append(arena, .{
                ...
                .value = try arena.dupe(u8, buf.bytes()),
            });
        }
        ...
        pub fn fromEnvMap(
            ...
            for (cmd.opts.items()) |opt| {
                ...
                defer if (prefix.len > 0) arena.free(key);
