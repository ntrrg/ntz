    ...
    return struct {
        ...
        pub fn load(
                ...
                switch (entry.from) {
                    ...
                    .flag => {
                        ...
                        if (loaded_from_flag and !opt.is_repeatable) {
                            ...
                            cmd.log.errf(arena, msg, .{entry.key});
                        ...
                        if (value.len > 0)
                            opt.validate(value) catch |err| {
                                ...
                                cmd.log.with("error", err).errf(arena, msg, .{ value, entry.key });
                            };
                    ...
                    .command, .argument => break,
                        ...
                        cmd.log.errf(arena, msg, .{ entry.key, @tagName(entry.from) });
            ...
            if (opt.is_required and action_entry == null) {
                cmd.log.err("option not specified");
            ...
            opt.validate(value) catch |err| {
                ...
                cmd.log.with("error", err).errf(arena, msg, .{ value, from, prefix, key });
            };
