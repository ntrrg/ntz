...
//! # `ntz.ui.cli`
//!
//! Command line interface implementation. Supports configuration files,
...
pub fn writeConfigStruct(
    writer: anytype,
    cmd: anytype,
    indent_level: usize,
) !void {
    if (indent_level == 0)
        _ = try writer.write("pub const Config = struct {\n");

    for (cmd.opts.items()) |opt| {
        if (opt.config.len == 0) continue;
        for (0..indent_level + 1) |_| _ = try writer.write("    ");
        _ = try writer.write(cmd.config_prefix);
        _ = try writer.write(opt.config);
        _ = try writer.write(": ");

        _ = try writer.write(
            if (opt.config_type.len > 0)
                opt.config_type
            else
                "[]const u8",
        );
    ...
    for (cmd.cmds.items()) |sub_cmd| {
        ...
        try writeConfigStruct(writer, sub_cmd, indent_level + 1);
        for (0..indent_level + 1) |_| _ = try writer.write("    ");
        _ = try writer.write("};\n");
    }

    if (indent_level == 0) _ = try writer.write("};\n");
}
